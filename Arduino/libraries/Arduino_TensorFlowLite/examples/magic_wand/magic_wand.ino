...
void loop() {
  ...
  Serial.print(F("Memory free at start of loop: "));
  Serial.println(checkMemory());
  ...
  error_reporter->Report("Got data: %s", got_data ? "true" : "false");
  // if (!got_data) return;
  // Run inference, and report any error
  // model_input->data.f[0] = -0.4;
  // model_input->data.f[1] = 0.9;
  ...
  Serial.print(F("NN output: "));
  Serial.println(*output);
  */
  // Analyze the results to obtain a prediction
  int gesture_index = PredictGesture(interpreter->output(0)->data.f);
  // error_reporter->Report("gesture_index: %d", gesture_index);
